...
import { constantTimeTokenEqual, FileOutbox, memoryNamespace, outboxDirectory, projectKey, projectNameFromDir, redactSensitiveValue, type StrataGateSnapshot } from '@diqier/stratagate'
import { SqliteStorage } from '@diqier/stratagate/sqlite'
import { resolveConfig, type WorkBuddyConfig } from './config.js'
...
export class MemoryGateway {
  ...
  async handle(req: IncomingMessage, res: ServerResponse): Promise<void> {
    ...
    try {
      ...
      if (req.method === 'GET' && (url.pathname === '/' || url.pathname === '/console' || url.pathname === '/v1/console')) return html(res)
      auth(req, this.token)
