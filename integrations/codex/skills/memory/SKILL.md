...
StrataGate registers MCP tools under the `mcp__stratagate__` prefix in Codex. Key ones:
...
- Hooks are gated behind hook trust. The installer writes the hooks into
  `~/.codex/config.toml`; approve/trust them when Codex prompts, or they will
  not run.
- In `codex exec` (headless) sessions hooks are not run; use the desktop
  interactive session for automatic recall/capture.
