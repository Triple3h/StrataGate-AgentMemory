  ...
  factory: (require) => {
    ...
    function ImportPage({ namespace, onBack, refresh }) {
      ...
      return h(React.Fragment, null,
        ...
        h('div', { className: 'sg-intro' }, h('h2', null, '导入别的 AI 记忆'), h('p', null, '粘贴外部 AI 按 StrataGate v2 格式导出的 JSON，导入到当前工作区。')),
        h('div', { className: 'sg-import-card' },
          h('textarea', { value: text, onChange: (event) => setText(event.target.value), placeholder: '{\n  "schemaVersion": "stratagate.external-memory.v2",\n  "candidates": [...]\n}', 'aria-label': '外部 AI 记忆 JSON' }),
          h('p', { className: 'sg-import-hint' }, '建议先让外部 AI 输出合法 JSON。导入会保留原始文本，并以保守的新增方式写入，后续可继续做合并或取代裁决。'),
          h('div', { className: 'sg-import-actions' }, h('button', { type: 'button', className: 'sg-import-button', disabled: busy || !text.trim(), onClick: submit }, busy ? '正在导入…' : '开始导入'), h('button', { type: 'button', className: 'sg-quiet-button', onClick: () => setText('') }, '清空')),
          error ? h('div', { className: 'sg-import-error' }, '导入失败：' + error) : null,
