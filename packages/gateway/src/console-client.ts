...
function projectBar() {
  ...
  return '<section class="scope-bar" aria-label="项目范围"><div class="path-field"><label class="sr-only" for="project-select">当前项目</label><span class="path-scheme">'+icon('database')+'stratagate://</span><select id="project-select" '+(!rows.length?'disabled':'')+'>'+
    ...
    '</select></div><div class="scope-meta">'+(current?(current.sourceAdapters || []).map(badge).join('')+'<span class="chip">'+esc(current.userId)+' · '+current.turns+' 轮</span>':'')+'</div></section>';
}
...
function empty(title, detail='', reset=false) { return '<div class="empty"><span class="empty-icon">'+icon('memory')+'</span><strong>'+esc(title)+'</strong>'+(detail?'<p>'+esc(detail)+'</p>':'')+(reset?'<button class="button" data-action="reset-filters">清除筛选</button>':'')+'</div>'; }
...
function sessionCards(rows) {
  return '<div class="session-cards">'+rows.map(row=>{
    const sources=unique(row.messages.map(m=>m.sourceAdapter)), agents=unique(row.messages.map(m=>m.agentId));
    const active=state.view==='conversations' && row.id===state.session;
    return '<a class="session-card'+(active?' active':'')+'" data-route href="'+esc(routeUrl('conversations',{session:row.id}))+'"><span class="card-icon">'+icon('sessions')+'</span><span class="card-body"><span class="card-title">'+esc(row.title || '历史对话')+'</span><span class="card-meta">'+(sources.length?sources.map(badge).join(''):'')+('<span>'+row.messages.length+' 消息 / '+row.messages.filter(m=>m.role==='user').length+' 轮</span>')+'</span><span class="card-sub"><span class="session-id" title="'+esc(row.id)+'">'+esc(row.id)+'</span><span class="muted">'+esc(fmt(row.last))+'</span></span></span></a>';
  }).join('')+'</div>';
}
function overview() {
...
function conversations() {
  ...
  const active=state.session ? rows.find(row=>row.id===state.session) : null;
  return heading('对话来源')+'<div class="session-split'+(active?' has-thread':'')+'"><aside class="session-list"><div class="toolbar session-toolbar">'+searchBox('搜索对话或消息内容')+'<div class="filters">'+filter(rows.flatMap(r=>r.messages.map(m=>m.agentId)),state.agentFilter,'Agent','agent-filter')+filter(rows.flatMap(r=>r.messages.map(m=>m.sourceAdapter)),state.sourceFilter,'来源','source-filter')+'</div><span class="count">'+filtered.length+' / '+rows.length+' 个对话</span>'+(hasFilters?iconButton('reset-filters','清除筛选','reset'):'')+'</div><div class="session-list-scroll">'+(filtered.length?sessionCards(filtered):empty(hasFilters?'没有匹配的对话':'当前项目还没有对话','',hasFilters))+'</div></aside><section class="thread-pane" aria-live="polite">'+(state.session?(active?threadView(active):empty('当前项目中未找到此对话')):sessionsEmpty())+'</section></div>';
}
function threadView(row) {
  const sources=unique(row.messages.map(m=>m.sourceAdapter)), agents=unique(row.messages.map(m=>m.agentId));
  return '<header class="thread-head"><div class="thread-title"><h2>'+esc(row.title || '对话详情')+'</h2><span class="scope-id" title="'+esc(row.id)+'">'+esc(row.id)+'</span></div><div class="thread-meta">'+(sources.length?sources.map(badge).join(''):'')+'<span class="chip">'+esc(agents.join('、') || '未知 Agent')+'</span><span class="chip">'+row.messages.length+' 消息 / '+row.messages.filter(m=>m.role==='user').length+' 轮</span>'+iconButton('close-session','返回对话列表','close')+'</div></header><div class="thread-scroll" id="thread-scroll">'+messageList(row.messages)+'</div>';
}
function sessionsEmpty() {
  return '<div class="thread-empty"><span class="empty-icon large">'+icon('compass')+'</span><strong>选择一个对话</strong><p>从左侧列表选择对话，这里会以聊天气泡展示完整的消息记录。</p></div>';
}
const statusTone = status => status==='failed'?'error':status==='running'?'info':['succeeded','completed','skipped','ready','active','organized'].includes(status)?'ok':'warn';
function blocks() {
...
function messageList(rows) {
  ...
  return rows.map(m=>{
    const isUser=m.role==='user', compact=previousRole===m.role;
    previousRole=m.role;
    return '<article class="msg'+(compact?' compact':'')+' '+(isUser?'msg-user':'msg-ai')+'">'+(isUser?'':'<span class="msg-avatar" aria-hidden="true">'+icon('brand')+'</span>')+'<div class="msg-body"><div class="bubble"><div class="msg-content">'+esc(m.content)+'</div></div><div class="msg-meta">'+badge(m.sourceAdapter || '未标注')+'<time class="muted">'+esc(fmt(m.createdAt))+'</time></div></div>'+(isUser?'<span class="msg-avatar user" aria-hidden="true">'+icon('user')+'</span>':'')+'</article>';
  }).join('');
}
function modal() {
  ...
  const title=row.title || row.name || ref, id=ref;
  const refs=row.sourceMessageIds || (row.sourceEventIds || []).flatMap(e=>s.events?.find(v=>v.id===e)?.sourceMessageIds || []);
  const body='<pre class="json">'+esc(JSON.stringify(row,null,2))+'</pre><section class="section"><h2>来源消息</h2>'+messageList(sourceMessages(refs))+'</section>';
...
function loginPage() {
  return '<main class="login"><section class="login-box"><a href="/dashboard" class="brand"><span class="logo">'+icon('brand')+'</span>StrataGate</a><h1>登录记忆控制台</h1><p class="login-sub">输入 Gateway Token 查看只读的记忆审计视图。</p><form id="login-form"><label for="token">Gateway Token</label><input id="token" name="token" type="password" autocomplete="off" required><button class="button primary" '+(state.loggingIn?'disabled':'')+'>'+(state.loggingIn?'正在验证…':'使用 Token 登录')+'</button></form>'+(state.error?'<div class="error" role="alert">'+esc(state.error)+'</div>':'')+'</section></main>';
}
const NAV_GROUPS=[['工作区',['overview','conversations','blocks','structure']],['运行',['processing','audit']],['配置',['import','settings']]];
function themePref() { return storage.get('stratagate_theme'); }
function applyTheme() {
...
function themeIcon() {
  const pref=themePref() || ((typeof window.matchMedia==='function' && window.matchMedia('(prefers-color-scheme: dark)').matches)?'dark':'light');
  return pref==='dark'?'sun':'moon';
}
function render() {
  ...
  const nav=NAV_GROUPS.map(([label,viewsInGroup])=>'<div class="nav-group"><span class="nav-label">'+esc(label)+'</span>'+viewsInGroup.map(view=>{const r=ROUTES.find(row=>row.view===view); return '<a class="nav-link '+(r.view===state.view?'active':'')+'" '+(r.view===state.view?'aria-current="page"':'')+' data-route href="'+esc(routeUrl(r.view,{query:'',session:''}))+'">'+icon(r.icon)+'<span>'+r.title+'</span></a>';}).join('')+'</div>').join('');
  root.innerHTML='<header class="top"><a class="brand" data-route href="'+esc(routeUrl('overview',{query:'',session:''}))+'"><span class="logo">'+icon('brand')+'</span>StrataGate</a><div class="top-status"><span class="chip"><span class="status-dot '+(state.error?'off':'')+'"></span>'+(state.error?'连接异常':'已连接')+'</span><span class="chip">只读审计</span>'+iconButton('theme','切换主题',themeIcon())+'</div></header><div class="layout"><aside class="sidebar"><nav aria-label="主导航">'+nav+'</nav></aside><main class="main" aria-busy="'+state.loading+'">'+projectBar()+(state.error?heading(route.title)+'<div class="error" role="alert">'+esc(state.error)+'</div>':'')+content+'</main></div>'+(!state.loading && !state.error?modal():'');
...
function closeDetail() { state.selected=null; render(); if(returnFocus) document.querySelector(returnFocus)?.focus(); }
document.addEventListener('click',event=>{
  ...
  if(action==='theme') { storage.set('stratagate_theme',themeIcon()==='sun'?'light':'dark'); applyTheme(); render(); return; }
  if(action==='close-session') void navigate(routeUrl('conversations',{session:''}));
