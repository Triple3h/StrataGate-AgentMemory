...
import { projectKey, type WorkBuddyConfig } from '../../adapter-sdk/src/config.js'
...
import {
  ...
  type StoredBatch,
} from '../../adapter-sdk/src/state.js'

import type { EvidenceItem, BatchResult, BlockQueryScope, BlockEmptyReason, RecordUseResult } from '../../adapter-sdk/src/contracts.js'
export type { EvidenceItem, BatchResult, BlockQueryScope, RecordUseResult } from '../../adapter-sdk/src/contracts.js'
...
function sessionIdFallback(): string {
  return process.env.CODEBUDDY_SESSION_ID?.trim()
    ...
    || process.env.STRATAGATE_SESSION_ID?.trim()
...
export class MemoryRuntime {
  ...
  constructor(readonly config: WorkBuddyConfig) {
    this.state = new AdapterState(config.dataDir)
  ...
  private emit(operation: Parameters<typeof observe>[1], outcome: Parameters<typeof observe>[2], startedAt: number, context: ObservationContext = {}, attributes: Parameters<typeof observe>[5] = {}): void {
    observe(this.observability, operation, outcome, {
      ...
      sourceAdapter: this.config.sourceAdapter ?? 'gateway',
      ...context,
  ...
  async appendTurn(input: Parameters<StrataGate['appendTurn']>[0]): Promise<unknown> {
    return this.withMemory((memory) => memory.appendTurn({
      ...
      sourceAdapter: input.sourceAdapter ?? this.config.sourceAdapter ?? 'gateway',
