...
import { z } from 'zod'
import { resolveConfig } from '../../adapter-sdk/src/config.js'
import { blockTarget, MemoryRuntime } from './runtime.js'
import { GatewayRuntime } from '../../adapter-sdk/src/gateway-client.js'
...
const runtime = !gatewayEnabled || process.env.STRATAGATE_DISABLE_GATEWAY === '1'
  ? new MemoryRuntime(config)
  : new GatewayRuntime(config)
const server = new McpServer({
  name: `stratagate-${config.sourceAdapter ?? 'gateway'}`,
  version: '0.1.0',
})
...
const session = z.string().min(1).max(200).optional().describe('Host session id; omit to use the current host session')
...
async function work(): Promise<void> {
  ...
  try {
    ...
    process.stderr.write(`[stratagate-${config.sourceAdapter ?? 'gateway'}] background processing failed: ${error instanceof Error ? error.message : String(error)}\n`)
  } finally {
    workerRunning = false
  }
}
...
void main().catch((error) => {
  process.stderr.write(`[stratagate-${config.sourceAdapter ?? 'gateway'}] MCP server failed: ${error instanceof Error ? error.message : String(error)}\n`)
  process.exitCode = 1
})
