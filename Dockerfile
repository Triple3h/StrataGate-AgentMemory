...
RUN npm run build:core && npm run build --workspace @diqier/stratagate-gateway && npm rebuild better-sqlite3 && npm prune --omit=dev
...
CMD ["node", "packages/gateway/dist/gateway.cjs"]
